...
func NewMessage(toEmail, senderName, messagePreview string) *Template {
	...
	return &Template{
		ToEmail: toEmail,
		Subject: "New message on Blindly",
		Text: fmt.Sprintf(`You have a new message on Blindly.

%s sent you a message:

"%s"

Open Blindly to reply and continue the conversation.`, senderName, messagePreview),
		HTML: baseTemplate(
			"New message",
...
func ProfileViewed(toEmail, viewerName string) *Template {
	return &Template{
		ToEmail: toEmail,
		Subject: "Someone viewed your profile on Blindly",
		Text: fmt.Sprintf(`Someone viewed your profile on Blindly.

%s checked out your profile.

Open Blindly to see who’s interested.`, viewerName),
		HTML: baseTemplate(
			"Profile view",
...
func SuperlikeReceived(toEmail, senderName string) *Template {
	return &Template{
		ToEmail: toEmail,
		Subject: "You received a superlike on Blindly",
		Text: fmt.Sprintf(`You received a superlike on Blindly.

%s showed strong interest in your profile.

Open Blindly to see more.`, senderName),
		HTML: baseTemplate(
			"Superlike received",
...
func NewMatch(toEmail, matchName string) *Template {
	return &Template{
		ToEmail: toEmail,
		Subject: "It’s a match on Blindly",
		Text: fmt.Sprintf(`It’s a match.

You and %s liked each other.

Start a conversation on Blindly.`, matchName),
		HTML: baseTemplate(
			"It’s a match",
...
func PokeReceived(toEmail, senderName string) *Template {
	return &Template{
		ToEmail: toEmail,
		Subject: "You received a poke on Blindly",
		Text: fmt.Sprintf(`You received a poke on Blindly.

%s wants to get your attention.

Open Blindly to respond.`, senderName),
		HTML: baseTemplate(
			"You got a poke",
...
<body style="margin:0;padding:32px;background:#0B0B10;font-family:-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,Helvetica,Arial,sans-serif;">
	<div style="max-width:480px;margin:0 auto;background:#121218;border-radius:16px;border:1px solid rgba(255,255,255,0.08);overflow:hidden;">
		...
		<div style="padding:28px 24px;text-align:center;background:linear-gradient(135deg,#6B21A8,#8B5CF6);">
			<h1 style="margin:0;color:#ffffff;font-size:24px;font-weight:700;">Blindly</h1>
		</div>
		...
		<div style="border-top:1px solid rgba(255,255,255,0.06);padding:16px;text-align:center;">
			<p style="margin:0;color:rgba(255,255,255,0.35);font-size:12px;">
				© 2025 Blindly · Personality first, always
			</p>
		</div>
	</div>
</body>
