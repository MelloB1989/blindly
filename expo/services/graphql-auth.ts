...
import { graphqlClient, setAccessToken, clearAuthTokens } from "./graphql-client";
import * as SecureStore from "expo-secure-store";
import { Platform } from "react-native";
import { gql } from "urql";

// Storage keys
const USER_KEY = "blindly_user";
...
class GraphQLAuthService {
     ...
     */
    async requestEmailLoginCode(email: string): Promise<{ success: boolean; error?: string }> {
        try {
            const result = await graphqlClient
