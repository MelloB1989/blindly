...
export default function PhotosScreen() {
    ...
    const pickImage = async () => {
        if (photos.length >= MAX_PHOTOS) {
            Alert.alert("Maximum Photos", `You can only upload ${MAX_PHOTOS} photos.`);
            return;
        }
    ...
    const handleContinue = async () => {
        ...
        try {
            ...
            if (updateResult.user) {
                const status = graphqlAuthService.getOnboardingStatus(updateResult.user);
        ...
        <SafeAreaView className="flex-1 bg-background">
            <View className="flex-1 px-6 py-6">
                ...
                <View className="absolute bottom-8 left-6 right-6 bg-background pt-4">
                    ...
                    <Button variant="ghost" size="sm" onPress={handleSkip} className="w-full">
